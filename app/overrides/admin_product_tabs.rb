...
Deface::Override.new(virtual_path: 'spree/admin/prices/_master_variant_table',
                     ...
                     text: '<th><%= Spree::Price.human_attribute_name(:orginal_price) %></th><th><%= Spree::Price.human_attribute_name(:sale_price) %></th>',
                     disabled: false)

Deface::Override.new(virtual_path: 'spree/admin/prices/_master_variant_table',
                     name: 'extend_price_table_with_original_and_sale_price_columns',
                     replace: "[data-hook='prices_row'] > td:nth-child(3)",
                     text: '<td><%= price.original_amount %></td><td><%= price.sale_amount %></td>',
                     disabled: false)
